...
import {BehaviorSubject, distinctUntilChanged, filter, interval, Subject} from "rxjs";
...
export class GenerateComponent implements OnInit, OnDestroy {
    ...
    sets: [
      ...
      },
      {
        ...
        questions: [{
          ...
          },
          {
            ...
            partText: 'and want something that will truly mimic a client project.',
            apiContext: 'develop',
            className: 'set3'
          }]
      },
      {
        ...
        questions: [{
          ...
          subText: 'Briefforge can generate a reference landing page to give you some inspiration',
  ...
  currentSet$: BehaviorSubject<QuestionSet> = new BehaviorSubject(this.questionData.sets[0]);
  constructor(
  ...
  nextSet() {
    ...
    if (hasNextStep && (hasNextStep.completed || (currentStep.completed && !hasNextStep.completed))) {
      this.currentSetIndex++
      console.log('navigated to step', this.currentSetIndex)
      this.currentSet$.next(this.questionData.sets[this.currentSetIndex]);
    }
  ...
  startTyping() {
    this.typingTrigger.subscribe(() => {
      if (this.currentPartIndex < this.overviewStringParts.length) {
        ...
        this.renderer.addClass(typingElement, this.overviewStringParts[this.currentPartIndex].className);
  ...
  completeSet($event: { part: string; className: string; id: number }) {
    ...
    if (!existingPart || existingPart.part !== $event.part) {
      ...
      this.overviewStringParts.splice(this.currentSetIndex, 1, { part: $event.part, className: $event.className });
      if (this.questionData.sets[this.currentSetIndex].completed) {
       this.typeReplacementWord({ part: $event.part, className: $event.className })
      }
